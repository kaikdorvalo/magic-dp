export const Errors = {
    INTERNAL_ERROR: 'Internal error.',
    USER_ALREADY_EXISTS: 'User already exists.',
    COMMANDER_NOT_FOUND: 'Commander not found.',
    NOT_COMMANDER: "This card isn't a valid commander.",
    INVALID_LANDS_AMOUNT: 'Invalid lands amount.',
    UNAUTHORIZED: "Unauthorized",
    DECK_NOT_FOUND: 'Deck not found.',
    EXPORT_DECK_ERROR: 'Export deck error.',
    USER_NOT_FOUND: 'User not found.',
    EMAIL_ALREADY_EXISTS: 'Email already exists.',
    WRONG_NUMBER_OF_CARDS: 'The deck must have exactly 100 cards',
    IMCOMPATIBLE_CARD_COLOR: 'Card color incompatible with commander',
    IMCOMPATIBLE_LAND_COLOR: 'Land color incompatible with commander'
}